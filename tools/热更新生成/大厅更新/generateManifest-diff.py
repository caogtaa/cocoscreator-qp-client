...
assetsDir = {
    #文件夹下需要进行热跟的文件夹
    "searchDir" : ["assets", "src","jsb-adapter"],
    #需要忽略的文件夹
    "ignorDir" : [".svn", ".git"],
    #需要忽略的文件
    "ignorFile":[".DS_Store"],
...
thisTimeUpdate = "version/thisTime.manifest" #对比两次版本需要更新的内容
tempprojectManifestPath = "project.manifest" #由此脚本生成的project.manifest临时文件路径

hallManifestPath = "res/project.manifest"
hallResPath = "assets"
hallSrcPath = "src"
hallJSBPath = "jsb-adapter"
...
class SearchFile:
    def __init__(self):
        ...
        self.fileList.append("main.js")
        self.fileList.append("project.json")
...
if __name__ == "__main__":
    if not os.path.exists(hallResPath):
        os.makedirs(hallResPath)
    if not os.path.exists(hallSrcPath):
        os.makedirs(hallSrcPath)
    if not os.path.exists(hallJSBPath):
        os.makedirs(hallJSBPath)

    #首先删除现在的文件
    if os.path.exists(hallResPath):
        shutil.rmtree(hallResPath)
    if os.path.exists(hallSrcPath):
        shutil.rmtree(hallSrcPath)
    if os.path.exists(hallJSBPath):
        shutil.rmtree(hallJSBPath)

    #拷贝更新的到当前目录["cocos", "framework", ".svn", ".git", "games"]
    shutil.copytree("../../" + hallResPath, hallResPath, False, shutil.ignore_patterns('.DS_Store', ".svn", ".git"))
    shutil.copytree("../../" + hallSrcPath, hallSrcPath, False, shutil.ignore_patterns('.DS_Store',".svn", ".git"))
    shutil.copytree("../../" + hallJSBPath, hallJSBPath, False, shutil.ignore_patterns('.DS_Store',".svn", ".git"))
    shutil.copyfile("../../main.js", "./main.js")
    shutil.copyfile("../../project.json", "./project.json")
    ...
    #shutil.copyfile(projectManifestPath, hallManifestPath)
