...
@ccclass
export default class TeamReportHhrLayer extends cc.Component {
    ...
    onLoad () {
        ...
        EventManager.getInstance().on('hall.response_team_report_partner_info', this.response_team_report_partner_info, this);
    }

    start () {
        UIManager.getInstance().showLoadingTip('正在加载，请稍候...');
        NetManager.getInstance().sendNetMsgToHall('hall', 'request_team_report_partner_info', {
    ...
    onDestroy() {
        EventManager.getInstance().off('hall.response_team_report_partner_info', this.response_team_report_partner_info, this);
    }

    // update (dt) {}

    private onClickClose() {
        UIManager.getInstance().removeLayer('hall', 'prefabs/TeamReportHhrLayer');
    }

    private response_team_report_partner_info(msg: hall.response_team_report_partner_info) {
