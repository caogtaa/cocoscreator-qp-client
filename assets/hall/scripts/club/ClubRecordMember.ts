...
@ccclass
export default class ClubRecordMember extends cc.Component {
    ...
    @property(cc.EditBox)
    private editboxGameID: cc.EditBox = null;
    @property(cc.Label)
    ...
    private onClickClearGameID() {
        this.editboxGameID.string = '';
    }
    ...
    private onClickSearch() {
        ...
        let gameid = 0;
        if (this.editboxGameID.string.length > 0 && this.editboxGameID.string.length != 8) {
            UIManager.getInstance().showToast('请输入8位的游戏ID');
            return;
        } else {
            gameid = parseInt(this.editboxGameID.string);
        }
        ...
        if (drawid != 0) {
            ...
            if (count == 0) {
                UIManager.getInstance().showToast('您搜索的玩家不存在');
            }
        } else {
    ...
    private loadOneItem(index: number, itemData: hall.Iteam_game_record_item) {
        ...
        item.getChildByName('LabelPlayerId').getComponent(cc.Label).string = itemData.gameid.toString();
